...
unsigned char* getSubBytesValue(int index) {
...
unsigned char* keyExpansion(const unsigned char* key, const int nbRounds) {
    unsigned char* completeKey = (unsigned char*)malloc((nbRounds + 1) * KEY_SIZE);
...
void addRoundKey(unsigned char* block, unsigned char* completeKey) {
...
int main() {
    unsigned char initialKey[16] = { 0x2b, 0x28, 0xab, 0x09, 0x7e, 0xae, 0xf7, 0xcf, 0x15, 0xd2, 0x15, 0x4f, 0x16, 0xa6, 0x88, 0x3c };
    unsigned char* completeKey = keyExpansion(initialKey, 10);
    for(int i = 0; i < 11 * (KEY_SIZE / 8); ++i) {
        printf("%x ", completeKey[i]);
    }
    return 0;
}